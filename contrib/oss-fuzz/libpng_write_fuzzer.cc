// libpng_full_write_fuzzer.cc  –  leak-free, wrap-safe version
#include <cstddef>
#include <cstdint>
#include <cstdlib>
#include <cstring>
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    /* ---- helpers ------------------------------------------------------- */
    size_t pos = 0;
    auto next_byte = [&]() -> uint8_t { uint8_t v = data[pos % size]; ++pos; return v; };
    ...
    /* ---- buffers that may leak if we don't free them ------------------ */
    uint8_t  *iccp = nullptr;                  /* iCCP profile */
    uint16_t *hist = nullptr;  int hist_n = 0; /* hIST data    */
    std::vector<png_bytep> rows;  size_t rows_alloc = 0;

    /* ---- longjmp handler: clean everything then return ---------------- */
    if (setjmp(png_jmpbuf(png_ptr))) {
        for (size_t i = 0; i < rows_alloc; ++i) free(rows[i]);
        if (hist) free(hist);
        if (iccp) free(iccp);
    ...
    if (flags & 0x0001) {                    /* PLTE */
        if (color_type == PNG_COLOR_TYPE_PALETTE || color_type == PNG_COLOR_TYPE_RGB) {
            int n = 1 + (next_byte() % 16);              /* 1‒16 entries */
            png_color pal[16];
            for (int i = 0; i < n; ++i) {
                pal[i].red   = next_byte();
    ...
    if (flags & 0x0002) {                    /* tEXt */
        ...
        for (int i = 0; i < 50; ++i) text_buf[i] = next_byte();
        txt.text = text_buf;
    ...
    if (flags & 0x0010) {                    /* gAMA */
        double gamma = 0.01 + (next_byte() / 255.0) * 10.0;
